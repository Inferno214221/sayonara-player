#include "SayonaraTest.h"
#include "Database/Connector.h"

#include "Utils/Utils.h"
#include "Utils/FileUtils.h"

SayonaraTest::SayonaraTest(const QString& test_name) :
	QObject(),
	mTmpPath(Util::temp_path(test_name))
{
	Util::File::create_directories(mTmpPath);

#ifdef SAYONARA_TEST_WITH_RCC
	Q_INIT_RESOURCE(Test);
	Q_INIT_RESOURCE(Database);
	Q_INIT_RESOURCE(Resources);

	DB::Connector::instance_custom("", mTmpPath, "");
#else
	qDebug() << "Test " << test_name << " runs without database";
#endif

	this->setObjectName(test_name);
}

SayonaraTest::~SayonaraTest()
{
	Util::File::delete_files({mTmpPath});
}

QString SayonaraTest::temp_path() const
{
	return mTmpPath;
}

QString SayonaraTest::temp_path(const QString& append) const
{
	QDir d(temp_path());
	return d.absoluteFilePath(append);
}
