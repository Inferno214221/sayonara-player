...
void GUI_Speed::init_ui()
{
	...
	ui->btn_speed->setText(QString::number(fSpeed, 'f', 2));

	ui->cb_preserve_pitch->setChecked( GetSetting(Set::Engine_PreservePitch));

	ui->sli_pitch->setValue(pitch * 10);
	...
	{ // mouse events for speed button
		...
		connect(mef_speed, &MouseEnterFilter::sig_mouse_entered, this, [=](){
			ui->btn_speed->setText(QString::number(1.0, 'f', 2));
		});

		connect(mlf_speed, &MouseLeaveFilter::sig_mouse_left, this, [=](){
			ui->btn_speed->setText(QString::number(ui->sli_speed->value() / 100.0, 'f', 2));
		});
	}
	...
	{ // mouse events for pitch button
		...
		connect(mlf_pitch, &MouseLeaveFilter::sig_mouse_left, this, [=](){
			ui->btn_pitch->setText(QString("%1 Hz").arg(ui->sli_pitch->value() / 10));
		});
	...
	connect(ui->tabWidget, &QTabWidget::currentChanged, this, &GUI_Speed::current_tab_changed);
...
{
	ui->btn_speed->setText(QString::number(val / 100.0, 'f', 2));
...
void GUI_Speed::pitch_changed(int slider_value)
{
...
void GUI_Speed::current_tab_changed(int idx)
{
